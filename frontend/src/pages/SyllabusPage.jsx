...
// т"?т"? Read user from localStorage т"?т"?
...
// т"?т"? Check if user is master@gmail.com т"?т"?
const checkMaster = () => {
...
// т"?т"? Tiny unique id helper т"?т"?
const uid = () => Math.random().toString(36).slice(2, 8);

// т"?т"? Check if user is trainer т"?т"?
const isTrainer = () => {
  const user = getLoggedInUser();
    ...
    >
      ...
      <div className="course-info">
        ...
        <div className="course-modules-count">
          <span>{course.modules?.length || 0} modules т?в {topicCount} topics</span>
        </div>
      </div>
...
// ============================================================
// QUIZ DATA (unchanged т?" not touched)
// ============================================================
const pythonQuizzes = {
  basics: [
  ...
  return (
    <div className="code-editor-wrapper">
      <div className="editor-toolbar">
        <span className="editor-lang">{language === 'python' ? 'dY?? Python' : 'dY-,я,? MySQL'}</span>
    ...
    <div className={`quiz-card ${result === 'correct' ? 'correct' : result === 'incorrect' ? 'incorrect' : ''}`}>
      <div className="quiz-card-header">
        <div className="quiz-meta">
          ...
          {isCompleted && <span className="quiz-done-badge">тo" Solved</span>}
        </div>
      ...
      <div className="quiz-workspace">
        ...
        <div className="quiz-controls">
          {quizType === 'python' && !pyodideReady && <span className="loading-badge">т?3 Loading Python runtime...</span>}
          <button className={`run-btn ${running ? 'running' : ''}`} onClick={handleRun} disabled={running || (quizType === 'python' && !pyodideReady)}>
            {running ? 'т?3 Running...' : 'т- Run Code'}
          </button>
        </div>
        {output !== '' && (
          <div className={`output-panel ${result}`}>
            <div className="output-header">
              <span>{result === 'correct' ? 'тo. Output (Correct!)' : result === 'incorrect' ? 'т?O Output (Not quite...)' : 'dY'е Error'}</span>
      ...
      {result === 'incorrect' && <div className="hint-banner">dY'б Not matching expected output. Check your logic and try again!</div>}
    </div>
  );
};
...
const QuizPlatform = ({ quizType }) => {
  ...
  const handleComplete = (qid) => {
    ...
    else if (total === 10) setAchievement({ icon: 'dY"е', msg: '10 Problems Solved! You\'re heating up!' });
    else if (total === 25) setAchievement({ icon: 'dY'Z', msg: 'Quarter Century! 25 Problems Done!' });
  };

  const levels = ['basics', 'intermediate', 'advanced'];
  const levelIcons = { basics: 'dYO▒', intermediate: 'тsб', advanced: 'dY"е' };
    ...
    <div className="quiz-platform">
      ...
      <div className="quiz-header">
        <div className="quiz-title-row">
          <span className="quiz-platform-icon">{quizType === 'python' ? 'dY??' : 'dY-,я,?'}</span>
            ...
            {questions.map((q, idx) => {
              ...
              return (
                <button key={q.id} className={`quiz-list-item ${active?'active':''} ${done?'done':''}`} onClick={() => setCurrentIdx(idx)}>
                  ...
                  {done ? <span className="qli-done">тo"</span> : active ? <span className="qli-active">т-║</span> : null}
                </button>
              );
            })}
          </div>
          ...
          {current && (
            <>
              ...
              <div className="quiz-nav-bar">
                <button className="qnav-btn" disabled={currentIdx===0} onClick={() => setCurrentIdx(i=>i-1)}>т+? Previous</button>
                <span className="qnav-info">{currentIdx+1} / {questions.length}</span>
                <button className="qnav-btn" disabled={currentIdx===questions.length-1} onClick={() => setCurrentIdx(i=>i+1)}>Next т+'</button>
              </div>
            </>
          )}
        </main>
...
const Celebration = ({ onClose }) => (
  <div className="celebration-overlay" onClick={onClose}>
    <div className="celebration-content" onClick={e => e.stopPropagation()}>
      ...
      <div className="celebration-badges"><span>dY?+</span><span>т-?</span><span>dYZ_</span></div>
...
const ModuleComplete = ({ moduleName, onNext }) => (
  <div className="module-complete">
    <div className="complete-icon">тo"</div>
    <h3>Module Complete!</h3>
    <p>You've finished <strong>{moduleName}</strong></p>
    <button className="btn btn-primary" onClick={onNext}>Next Module т+'</button>
  </div>
);

// ============================================================
// т"?т"? TRAINER EDITOR COMPONENTS т"?т"?
// ============================================================

// Inline editable text т?" double-click to edit
const InlineEdit = ({ value, onSave, className = '', multiline = false, placeholder = 'Click to edit...' }) => {
  ...
  return (
    <span className={`trainer-editable ${className}`} onDoubleClick={() => { setDraft(value); setEditing(true); }} title="Double-click to edit">
      ...
      <span className="trainer-edit-hint">тo?я,?</span>
    </span>
  );
};
...
const ConfirmModal = ({ message, onConfirm, onCancel }) => (
  <div className="confirm-overlay">
    <div className="confirm-modal">
      <div className="confirm-icon">тsая,?</div>
    ...
    <div className="content-editor-overlay">
      <div className="content-editor-modal">
        <div className="cem-header">
          <h2>тo?я,? Editing: <span style={{color:'#00d9ff'}}>{topic}</span></h2>
          <button className="cem-close" onClick={onClose}>тo</button>
        </div>
          ...
          {draft.sections.map((sec, idx) => (
            <div key={idx} className="cem-section-block">
              <div className="cem-section-controls">
                ...
                <button className="cem-move-btn" onClick={() => moveSection(idx, -1)} disabled={idx===0}>т+`</button>
                <button className="cem-move-btn" onClick={() => moveSection(idx, 1)} disabled={idx===draft.sections.length-1}>т+"</button>
                <button className="cem-remove-section" onClick={() => removeSection(idx)}>тo</button>
              </div>
...
// ============================================================
// TRAINER PANEL т?" Syllabus Structure Editor (sidebar panel)
// ============================================================
    ...
    <div className="trainer-panel">
      ...
      <div className="trainer-panel-header">
        <div className="tph-title">
          <span className="tph-icon">тsTя,?</span>
          <span>Syllabus Editor</span>
        </div>
        <button className="tph-close" onClick={onClose}>тo</button>
      </div>
        ...
        {Object.entries(subject.modules).map(([moduleName, topics]) => (
          <div key={moduleName} className="tp-module-block">
            <div className="tp-module-header">
              <span className="tp-module-drag">та┐</span>
              <InlineEdit
                value={moduleName}
            ...
            <div className="tp-topics-list">
              {topics.map((topic, tIdx) => (
                <div key={topic + tIdx} className="tp-topic-row">
                  <span className="tp-topic-bullet">т-+</span>
                  <InlineEdit
                    value={topic}
                  ...
                  <div className="tp-topic-actions">
                    <button className="tp-btn-sm" onClick={() => moveTopic(moduleName, tIdx, -1)} disabled={tIdx===0} title="Move up">т+`</button>
                    <button className="tp-btn-sm" onClick={() => moveTopic(moduleName, tIdx, 1)} disabled={tIdx===topics.length-1} title="Move down">т+"</button>
                    <button className="tp-btn-sm tp-del-sm" onClick={() =>
                      setConfirm({ message: `Delete topic "${topic}"?`, action: () => deleteTopic(moduleName, topic) })
                    } title="Delete">тo</button>
                  </div>
                </div>
              ))}
    ...
    <>
      ...
      <div className="mode-tabs">
        ...
        {isT && viewMode === 'courses' && (
          <div className="trainer-badge-row">
            <span className="trainer-badge">тsTя,? Trainer Mode</span>
            <button
              className={`trainer-edit-toggle ${trainerPanelOpen ? 'active' : ''}`}
              onClick={() => setTrainerPanelOpen(p => !p)}
              title="Toggle syllabus editor"
            >
              {trainerPanelOpen ? 'тo Close Editor' : 'тo?я,? Edit Syllabus'}
            </button>
            <button className="trainer-reset-btn" onClick={handleResetSyllabus} title="Reset to defaults">т+║ Reset</button>
            {savedIndicator && <span className="saved-indicator">тo" Saved</span>}
          </div>
        )}
      </div>
      ...
      {viewMode === 'courses' && (
        <>
          ...
          {/* т"?т"? DETAILED COURSE VIEW (when a topic is selected) т"?т"? */}
                ...
                {!sidebarCollapsed && (
                  <aside className="course-sidebar-wrap">
                    <div className="sidebar-header">
                      ...
                      <button className="collapse-btn" onClick={() => setSidebarCollapsed(true)}>т+?</button>
                    </div>
                        ...
                        return (
                          <div key={moduleName} className="module-group">
                            <button className={`module-header ${isModuleActive ? 'active' : ''} ${isModuleComplete ? 'completed' : ''}`} onClick={() => setActiveModule(isModuleActive ? null : moduleName)}>
                              <span className="module-icon">{isModuleComplete ? 'тo"' : isModuleActive ? 'т-╝' : 'т-'}</span>
                                ...
                                {topics.map((topic, idx) => {
                                  ...
                                  return (
                                    <button key={topic} className={`topic-item ${isActive ? 'active' : ''} ${isCompleted ? 'completed' : ''}`} onClick={() => handleTopicClick(topic, moduleName)} style={{ animationDelay: `${idx * 0.05}s` }}>
                                      <span className="topic-bullet">{isCompleted ? 'тo"' : isActive ? 'т-+' : 'т-+'}</span>
                                      <span className="topic-text">{topic}</span>
                                      {isMasterUser && hasCustomContent && <span className="custom-content-dot" title="Has custom content">т-?</span>}
                    ...
                    <div className="welcome-screen">
                      ...
                      {isT && (
                        <div className="trainer-welcome-hint">
                          <span>тsTя,? Trainer:</span> Use <strong>тo?я,? Edit Syllabus</strong> above to manage modules & topics. Click any topic then use <strong>Edit Content</strong> to update its body.
                        </div>
                      )}
                      <div className="start-hint"><span className="hint-arrow">т+?</span><span>Choose a module to begin</span></div>
                    </div>
                  ) : (
                    <div className="content-display">
                      <div className="content-header">
                        <div className="breadcrumb">
                          <span>{currentSubject.title}</span><span>т?║</span><span>{activeModule}</span><span>т?║</span>
                        ...
                        <div className="content-actions">
                          <button className={`action-btn ${completedTopics.has(activeTopic) ? 'completed' : ''}`} title="Mark Complete"
                            ...
                            {completedTopics.has(activeTopic) ? 'тo"' : 'т-<'}
                          </button>
                          <button className="action-btn" title="Bookmark">dY"-</button>
                          <button className="action-btn" title="Share">т+-</button>
                          {masterUser && (
                          <button
                      ...
                      <div className="content-footer">
                        <button className="nav-btn prev" onClick={handlePrevious} disabled={!canGoPrevious}>т+? Previous</button>
                        ...
                        <button className="nav-btn next" onClick={handleNext} disabled={!canGoNext}>
                          {completedTopics.size === allTopics.length - 1 ? 'Finish dYZ%' : 'Next т+''}
                        </button>
                      </div>
                    </div>
                  )}
      ...
      <style>{`
        /* т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?
          ...
          The page is divided into 3 horizontal bands:
            1. .mode-tabs          т?" sticky, ~44px
            2. .subject-tabs       т?" static, ~46px
            3. .course-progress-bar т?" static, 3px
            4. .course-layout-root т?" fills EXACT remaining viewport height
          ...
          own internal scroll. The content column is a flex-column:
            ...
            middle: .content-body    (flex:1, overflow-y:auto т?" SCROLLS)
            bottom: .content-footer  (always visible, never scrolls)
          
          This ensures footer is ALWAYS visible regardless of content
          length т?" it is NOT position:sticky (which depends on scroll
        ...
        .courses-page {
          ...
          width: 100%;
          /* Exactly fill the viewport т?" no more, no less */
          height: 100vh;
          overflow: hidden;
          /* Navbar is 52px т?" offset so we start below it */
          padding-top: 52px;
          margin-top: -52px;
        }
        ...
        /* т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?
           COURSE SIDEBAR т?" fixed width, full height, scrollable
        т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"?т"? */
        .course-sidebar-wrap {
          width: 280px;
          min-width: 280px;
        ...
        /* Sidebar header т?" fixed at top of sidebar */
        .sidebar-header {
          display: flex; align-items: center; justify-content: space-between;
        ...
        .content-area {
          ...
          min-width: 0;
          min-height: 0;
          overflow: hidden;         /* NOT auto т?" children scroll themselves */
          display: flex;
          flex-direction: column;
        }
        ...
        /* Welcome screen т?" scrollable within content-area */
        .welcome-screen {
          flex: 1;
          display: flex; flex-direction: column;
        ...
        /* Content display т?" fills content-area completely */
        .content-display {
          display: flex;
          flex-direction: column;
          width: 100%;
        ...
        .content-header {
          ...
          flex-shrink: 0;     /* never shrink т?" always visible */
        ...
        /* Content body т?" this is the ONLY scroll zone */
        .content-body {
        ...
        /* Content footer т?" ALWAYS visible, pinned at bottom of flex column */
        .content-footer {
          display: flex; align-items: center; justify-content: space-between;
          padding: 14px 28px;
          border-top: 1px solid rgba(255,255,255,0.08);
          background: rgba(6, 0, 20, 0.97);
          flex-shrink: 0;    /* never shrink т?" always takes its full height */
          flex-wrap: wrap; gap: 12px;
          /* NOT position:sticky т?" that requires scroll parent to have overflow:auto */
        ...
        /* т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?
        3D FLOATING QUIZ ORB т?" Attention Grabbing Edition
      ...
      .quiz-orb-wrap {
        position: fixed;
        bottom: 32px;
        right: 110px;   /* т+? moves it left of the tools orb (60px orb + 32px right + 18px gap) */
        left: unset;
        z-index: 9999;
        display: flex;
        flex-direction: column;
      ...
      /* т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?
        MAIN ORB т?" 3D Sphere with Depth & Glow
      т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т? */
      ...
      /* Hover state т?" intensify everything */
      .quiz-main-orb:hover {
      ...
      /* Open state т?" morph and spin fast */
      .quiz-main-orb.open {
      ...
      /* Active quiz state т?" pulsing alert */
      ...
      /* Smooth floating motion т?" like it's suspended in liquid */
      ...
      /* Alert pulse when quiz is active т?" attention grabbing */
      ...
      /* т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?
        ORBITING PARTICLES т?" Extra visual flair
      т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т? */
      ...
      /* т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?
        FAN MENU т?" Smooth unfold with 3D depth
      т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т? */
      ...
      /* т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?
        FAN LABELS т?" Glass morphism style
      т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т? */
      ...
      /* т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?
        TOOLTIP т?" Floating label under main orb
      т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т? */
      ...
      /* т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?
        ATTENTION GRABBER т?" Subtle screen edge glow
      т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т?т? */
      ...
      {/* т"?т"? QUIZ ORB т?" bottom-left floating brain orb т"?т"? */}
      <div className="quiz-orb-wrap">
        <div className={`quiz-orb-fan ${quizOrbOpen ? 'open' : ''}`}>
          {[
            ...
            { id: 'quiz-mysql',  label: 'MySQL Quiz',  icon: 'dY-,я,?', color: '#a855f7' },
          ].map((t, i) => (
            <div
              key={t.id}
        ...
        <button
          ...
          title="Quizzes"
        >
          {quizOrbOpen ? 'тo' : (viewMode==='quiz-python'||viewMode==='quiz-mysql') ? 'т?"' : 'dYа'}
